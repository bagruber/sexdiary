...
`)}
    </div>`,a=e=>`    <ol class="ablauf">
${e.map(e=>`      <li${e.offen?` class="offen"`:``}>
        <p class="tat">${e.tat}</p>
        <p class="folge">${e.folge}</p>
      </li>`).join(`
`)}
    </ol>`,o=e=>`    <ul class="module">
${e.map(e=>`      <li>
        <h3>${e.name} <span class="stand">${e.stand}</span></h3>
        <p>${e.text}</p>
      </li>`).join(`
`)}
    </ul>`,s=e=>`    <p class="fehlt"><strong>Fehlt noch:</strong> ${e}</p>`,c=e=>`
    <defs>
...
${r(`julia`,`Julia will jemanden erreichen, den sie nicht mehr sehen will`,`    <p class="vorspann">
      Julia und Stefan hatten einmal Sex und danach keinen Kontakt mehr. Daran
      will sie nichts ändern. Sie will ihn nur erreichen.
    </p>
${a([{tat:`Der Abend wird ein Eintrag.`,folge:`Datum, welche Praktiken, ob geschützt. Stefan wird ein Kontakt ohne Namen — ein Kürzel, das nur sie versteht. Ab diesem Datum läuft für jede Infektion ihr eigenes diagnostisches Fenster: die Rechnung, die kaum jemand im Kopf hat.`},{tat:`Wochen später der Test.`,folge:`Das Ergebnis kommt als QR-Code, sie liest ihn ein. Chlamydien positiv, alles Übrige ohne Befund. Von Hand eintragen geht genauso — der Code spart Tippen und bringt die Signatur der Teststelle mit, geprüft wird sie auf dem Gerät.`},{tat:`Die App rechnet rückwärts.`,folge:`Nicht „wen kennst du“, sondern: welche Begegnungen liegen in dem Zeitraum, in dem genau diese Infektion weitergegeben worden sein kann? Es sind zwei.`},{tat:`Sie entscheidet je Kontakt, wie.`,folge:`Den einen ruft sie an; sie kennt ihn gut, ein Anruf ist der bessere Weg. Die App hält fest, dass er Bescheid weiß, und hört auf, ihn anzumahnen. Persönlich ist hier kein Kanal zweiter Klasse.`},{tat:`Stefan bekommt die anonyme Benachrichtigung.`,folge:`Vor dem Senden steht auf dem Bildschirm, was übertragen wird: sein Token, das Wort „Chlamydien“, ein Zeitstempel. Kein Absender, kein Text, kein Weg zurück zu ihr. Wenn sie will, legt sie eine Rückadresse bei — dann kann er antworten, mit einem von vier Wörtern, mehr gibt es nicht.`},{offen:!0,tat:`Offen: Stefan hat die App nicht.`,folge:`Von App zu App trägt der Weg, sobald der Dienst steht. Über eine Telefonnummer ist er vorgesehen — aber es wäre der einzige Punkt im ganzen Entwurf, an dem ein Server etwas erführe, das eine Person bezeichnet. Deshalb steht er unter „Was dazukommt“ und nicht in der Basis: erst die rechtliche Klärung, dann der Code.`}])}`)}
${r(`max`,`Max tauscht kein Wort und trotzdem etwas aus`,`    <p class="vorspann">
      Max geht cruisen. Es fällt kein Name, es wird keine Nummer getauscht, und
      daran soll sich nichts ändern.
    </p>
${a([{tat:`Einer der Männer hat eine Karte.`,folge:`Max hält sein Telefon dran, eine Sekunde. Getauscht wird eine Zufallszahl — kein Name, keine Nummer, kein Profil, keine Verbindung zu irgendeinem Konto, auf keiner Seite.`},{tat:`Später trägt er nach, was war.`,folge:`Anal rezeptiv, ohne Kondom. Der Schutz wird je Praktik erfasst, nicht pauschal für den Abend. Der Kontakt heißt „Karte, Donnerstag“ oder gar nichts.`},{tat:`Die App sagt ihm nicht, wie gefährlich das war.`,folge:`Sie sagt, ab wann welcher Test etwas aussagt, und woraus sich das ergibt. Ein Test vor seinem Fenster ist ein Test ohne Aussage — das ist der eigentliche Nutzen.`},{tat:`Einige Wochen später eine Erinnerung.`,folge:`Auf dem Sperrbildschirm steht keine Infektion, keine Zahl, kein Datum. Dort liest die Person mit, vor der diese App schützt. Erst dahinter, entsperrt: Syphilis ist ab heute aussagekräftig testbar.`},{tat:`Und andersherum.`,folge:`Bekäme der Mann mit der Karte einen Befund, erreichte er Max über dieselbe Zufallszahl — ohne je erfahren zu haben, wer Max ist.`}])}`)}
${r(`motive`,`Drei Gründe, das zu benutzen`,`    <p>
      Sie hängen nicht voneinander ab. Der erste trägt allein, auch wenn nie
      jemand eine Benachrichtigung sendet.
    </p>
${i([[`Den Überblick behalten`,`Der Nutzen beginnt beim ersten Eintrag. Wer weiß, wann ein Test etwas aussagt, testet zum richtigen Zeitpunkt statt zu früh, zu oft oder gar nicht.`],[`Jemanden erreichen, ohne sich zu zeigen`,`Das Hindernis ist die Hemmschwelle, nicht die Technik. Wer nicht anrufen kann oder will, unterlässt es heute meistens ganz.`],[`Und dafür nicht mit Daten bezahlen`,`So wenig auf einem Server wie möglich, und was dort liegt, in öffentlicher statt privater Hand. Auf dem Gerät eine Darstellung, die auch dann nichts verrät, wenn jemand danebensteht.`]])}`)}
${r(`basis`,`Ohne diese drei ist es nicht das Produkt`,o([{name:`Kalender`,stand:`gebaut`,text:`Begegnungen, Tests, Impfungen und Prophylaxen in einer Zeitachse und einer Monatsansicht. Nachträglich änderbar und löschbar, weil ein Protokoll, das sich nicht korrigieren lässt, nicht geführt wird.`},{name:`Ein Dashboard, das mitrechnet`,stand:`gebaut`,text:`Was heute testbar ist, was noch wartet, und woraus sich das ergibt. Keine unerklärte Zahl: jede Aussage lässt sich auf den Eintrag zurückführen, der sie erzeugt hat.`},{name:`Anonyme Benachrichtigung über ein Relay`,stand:`Ablauf gebaut, Dienst fehlt`,text:`Von App zu App über beidseitig getauschte Token. Ablauf, Format und Rückmeldung stehen in der App; der Dienst, der die Nachricht hält, ist das größte offene Stück.`}]))}
${r(`dazu`,`Was dazukommt`,`    <p>
...
${o([{name:`QR-Codes für Tests und Kontakte`,stand:`gebaut, Signaturprüfung nicht angeschlossen`,text:`Ergebnisse und Kontakte wandern per Code statt per Tippen. Format und Prüfverfahren liegen im Kern und sind getestet — im Scanpfad wird die Signatur noch nicht geprüft. Bis das steht, ist ein eingelesenes Ergebnis nicht mehr wert als ein von Hand eingetragenes.`},{name:`NFC-Karten für Kontakte`,stand:`gebaut, auf einem Gerät nicht erprobt`,text:`Eine Karte, die eine Zufallszahl trägt. Für Orte, an denen ein Telefon herauszuholen unpassend ist, und für Menschen, die keines herausholen wollen.`},{name:`Personalisierung`,stand:`teilweise gebaut`,text:`Anatomie und Region bestimmen, welche Zahlen überhaupt gelten. Ohne diese Angabe rechnet die App mit dem ungünstigsten Fall — sicher, aber es warnt Menschen vor Wegen, die sie nicht gehen.`},{name:`Diskretion auf dem Gerät`,stand:`gebaut`,text:`Sperre, kein Bildschirmfoto, neutraler Name im Betriebssystem, inhaltsleere Erinnerungen. Was davon warum nötig ist, steht im nächsten Abschnitt.`},{name:`Weiterführende Information`,stand:`in Ansätzen`,text:`Zu Infektionen, Symptomen und Testmöglichkeiten — an der Stelle, an der jemand gerade etwas wissen will, statt als Broschüre.`},{name:`Anbindung an die Testinfrastruktur`,stand:`Konzept`,text:`Termin buchen, Befund abrufen. Der Abruf läuft zwischen Gerät und Teststelle; ein Betreiber der App steht nicht dazwischen und erfährt nichts.`},{name:`Hinweis bei örtlichem Geschehen`,stand:`Konzept`,text:`Wenn eine Infektion in einer Region gerade auffällig oft auftritt. Der Hinweis geht an alle gleich und trägt keinen Personenbezug — die App fragt ihn ab, sie meldet sich nicht an.`},{name:`Benachrichtigung an eine Telefonnummer`,stand:`rechtlich ungeklärt`,text:`Der Fall Stefan: der Empfänger hat die App nicht. Technisch die kleinere Aufgabe, rechtlich die größere — der Dienst bekäme eine Rufnummer zu sehen, müsste sie an einen Versanddienstleister weiterreichen, und der Empfänger hat in nichts davon eingewilligt. Steht deshalb hier und nicht in der Basis.`}])}`)}
...
${r(`architektur`,`Lokal denken, minimal zentralisieren`,`    <p>
...
${r(`bausteine`,`Bausteine und Abhängigkeiten`,`    <p>
...
${r(`server`,`Was der Server können muss, und was er nicht dürfen darf`,`    <p>
...
${r(`offen`,`Klare Augen: was noch nicht gelöst ist`,`    <h3>Medizinisch — das Dringendste</h3>
    <p>
...
${s(`Bezugsweg für die Android-App, sobald die Verteilung steht.`)}`)}
${r(`teststellen`,`Wo man sich testen lassen kann`,s(`Verzeichnis der Teststellen. Gehört mit geprüften Angaben gefüllt, etwa vom Gesundheitsreferat oder der Aidshilfe.`))}
${r(`impressum`,`Impressum`,s(`Anbieterkennzeichnung nach § 5 DDG: Name, ladungsfähige Anschrift, Kontakt.`))}
...
${s(`Erklärung nach Art. 13 DSGVO samt Verantwortlichem, Rechtsgrundlagen und Betroffenenrechten. Gehört juristisch geprüft.`)}`)}
</main>

<footer>
  <p>Kein Medizinprodukt. Ersetzt keine ärztliche Beratung.</p>
</footer>`}var f=document.getElementById(`root`);if(!f.firstElementChild){let e=document.createElement(`style`);e.textContent=n(),document.head.append(e),f.innerHTML=d()}function p(e){document.documentElement.dataset.theme=e,document.getElementById(`theme`).onclick=()=>{p(e===`dark`?`light`:`dark`)}}p(window.matchMedia?.(`(prefers-color-scheme: dark)`).matches?`dark`:`light`);