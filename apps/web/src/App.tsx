...
import type { AppData, Contact, TestRecord } from "@sexdiary/core";
import { parseBackupPayload, parseImportPayload } from "@sexdiary/core";
...
function AppShell() {
  ...
  const importFile = () => {
    ...
    input.onchange = async () => {
      ...
      // Full backup first; every row is validated and malformed rows dropped.
      ...
      if (backup.kind === "backup") {
        dispatch({
          ...
          payload: {
            ...data,
            contacts: backup.data.contacts ?? data.contacts,
            intercourse: backup.data.intercourse ?? data.intercourse,
            tests: backup.data.tests ?? data.tests,
            vaccinations: backup.data.vaccinations ?? data.vaccinations,
            profile: backup.data.profile ?? data.profile,
          },
        });
        return;
      }
      ...
      if (r.kind === "test") dispatch({ type: "saveTest", payload: r.record });
